...
The argument vector being parsed — the same slice [`lib.New`](/docs/References/PublicApi/lib.New.md) was called with. Every index-based function field (`GetStringArg`, `GetStringOption`, `GetStringKeyValues`, and their typed variants) refers to positions in this slice — index `0` is the first argument after the program name.
