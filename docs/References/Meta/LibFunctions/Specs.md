...
- `sandbox/` is a closed sandbox: a factory must never import `examples/`, a third-party module, or an OS-bound standard-library package (`os`, `net`, `syscall`, …). See [SandboxIsolation.md](/docs/Explanations/SandboxIsolation.md).
- A closure returning a library object returns that object's **api struct**, built by the object package's `New` constructor — see the [LibObjects](/docs/References/Meta/LibObjects/Specs.md) specification.
...
4. **`New` constructor**: `func New(...) api.Lib` building `api.Lib{...}`, calling every field factory in the package exactly once and assigning its return value into the matching field, and returning the struct. `sandbox/new.go` does nothing but delegate to it.
