...
- The object's api struct declares a `Deps deps.Deps` field plus its properties — see the [Outputs](/docs/References/Meta/Outputs/Specs.md) specification.
...
- Factories reach dependencies only through the object's `Deps` field, calling them as function fields: `o.Deps.<Field>(...)`. `sandbox/` is a closed sandbox: an object package must never import `adapters/`, `examples/`, a third-party module, or an OS-bound standard-library package (`os`, `net`, `syscall`, …). See [SandboxIsolation.md](/docs/Explanations/SandboxIsolation.md).
...
## Structure
1. **Struct declaration** (in `sandbox/contracts/api/api.go`): a `Deps deps.Deps` field, the object's data fields, and the function fields it exposes — see [Outputs](/docs/References/Meta/Outputs/Specs.md).
...
4. **Constructor factory** (in `sandbox/internal/lib/`): `func New<Object>Factory(l *api.Lib) func(...) api.<Object>` whose returned closure calls `<object>.New(l.Deps, …)`.
