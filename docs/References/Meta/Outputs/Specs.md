...
- `api.go` may import `sandbox/contracts/deps`, and must not import anything from `adapters/`, `examples/`, `sandbox/internal/`, or `sandbox` (the entry point) — the contract stays free of implementations.
